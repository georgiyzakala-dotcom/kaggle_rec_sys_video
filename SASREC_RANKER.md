...
Обновление2026-09-09: полный run завершён пользователем; final artifact и CSV
проверены повторно. Canonical P20 all/labeled —0.005333946200887327 /
0.007260806877227347; rolling_3 —0.007672918581877774 / 0.009515415499764554.
Обе full-user оценки выше Task13; подтверждения leaderboard score пока нет.
Рабочий кэш после завершения можно проверить командой
`./scripts/cleanup_task15_ranker.sh` и удалить с `--apply`.
На G: основное место возвращает сжатие VHDX, а не повторное удаление уже
очищенных training pools. Команды и проверки описаны в [DISK_CLEANUP.md](DISK_CLEANUP.md).
