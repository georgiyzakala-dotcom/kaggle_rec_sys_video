...
run_id="task10_catboost_ltr_v1"
config_path="configs/task10_catboost_ltr_v1.json"
...
minimum_free_gib="${TASK10_MIN_FREE_GIB:-420}"
...
if ((available_kib < required_kib)); then
    echo "ERROR: less than ${minimum_free_gib} GiB is available for Task 10." >&2
    exit 1
fi
...
if ((available_ram_kib < required_ram_kib)); then
    echo "ERROR: less than ${minimum_available_ram_gib} GiB RAM is available for Task 10." >&2
    exit 1
fi
...
echo "Task 10 bounded CatBoost LTR selection is starting."
echo "Artifact: ${output_dir}"
echo "Resume checkpoint: ${checkpoint_dir}/checkpoint.json"
echo "Portable best model: ${best_model_dir}/best_model.json"
echo "Reusable grouped pools: artifacts/task10_catboost_grouped_pools_*_v1"
echo "Compact rotating log: ${log_file}"
echo "Resource thresholds: RAM=${minimum_available_ram_gib} GiB, disk=${minimum_free_gib} GiB, GPU free=${minimum_gpu_free_mib} MiB"
