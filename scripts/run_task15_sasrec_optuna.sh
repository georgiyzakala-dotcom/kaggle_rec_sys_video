...
cd "${sasrec_repo}"
if [[ "${1:-}" == "--help" ]]; then
    exec .venv/bin/python scripts/run_sasrec_optuna.py --help
fi
sasrec_config="${1:-configs/task15_sasrec_optuna_v1.json}"
if (( $# > 0 )); then shift; fi
...
printf 'SASRec Optuna: %s\nOutput: artifacts/%s/metrics.json\nLog: logs/%s.log\n' "${sasrec_run}" "${sasrec_run}" "${sasrec_run}"
printf 'Stop: Ctrl-C or kill -TERM $(cat %s)\nRepeat this command to resume from the last completed trial/epoch.\n' "${sasrec_pid_file}"
.venv/bin/python scripts/run_sasrec_optuna.py --config "${sasrec_config}" "$@" &
sasrec_child=$!
