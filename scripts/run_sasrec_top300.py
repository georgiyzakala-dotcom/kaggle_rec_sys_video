#!/usr/bin/env python3
"""Recompute fold metrics and evaluate ALS300/SASRec300; inference only."""
...
IMPLEMENTATION = (
    *FOLD_IMPLEMENTATION,
    "sasrec_top300.py",
    "scripts/run_sasrec_top300.py",
)


def load_config(path):
    c = read_json(path)
    if c["kind"] != "sasrec_top300_evaluation" or not re.fullmatch(
        r"[A-Za-z0-9][A-Za-z0-9_-]*", c["run_id"]
    ):
        raise ValueError("invalid top300 kind/run ID")
    ...
    if c["mode"] == "full":
        ...
        s = c["smoke"]
        if not (1 <= s["shards_per_fold"] <= 2 and 1 <= s["users_per_shard"] <= 64):
...
def resolve_config(c):
    ...
    c["library_versions"] = {"torch": str(torch.__version__), "polars": pl.__version__}
    return c


class Top300Runner(FoldsRunner):
    ...
    def run_fold(self, fold):
        ...
        for path, digest in info["file_sha256"].items():
            if path.endswith("candidates.parquet"):
                self.check_stop()
                if sha256_file(path) != digest:
                    raise ValueError(f"fixed source changed: {path}")
        ...
        with self.bar(len(parts), f"{name} | SASRec300 + metrics") as bar:
            for index, old in enumerate(parts):
                ...
                current = summarize_extension(stats, c["recipe"]["source_policy"])
                partial = parts_dir / (old.name + ".partial")
                clean_partial(partial)
                deeper.write_parquet(partial / "sasrec300.parquet")
        ...
        measured = summarize_extension(stats, c["recipe"]["source_policy"])
        if c["mode"] == "full":
...
def verify_artifact(path):
    ...
    for fold in c["folds"]:
        ...
        if not metrics_equal(measured, metric["folds"][fold["name"]]):
            raise ValueError("published top300 metric aggregation differs")
    ...
    with Path("experiments/results.csv").open("a+", newline="") as handle:
        ...
        for name, m in metric["folds"].items():
            row = {
                ...
                "split": name + "_24h_top300_diagnostic",
                "seed": c["seed"],
                ...
                "notes": "inference only; union budget1200 vs1000; old metrics independently recomputed; no ALS600 control",
            }
...
def main(argv=None):
    ...
    parser.add_argument(
        "--config", type=Path, default=Path("configs/task15_sasrec_top300_v1.json")
    )
